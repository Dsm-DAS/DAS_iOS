...
class CommentPlusVC: BaseVC {
    var feedId = 0
    let viewModel = CommentPlusViewModel()
    private let titleLabel = UILabel().then {
        $0.text = "댓글"
        $0.textColor = UIColor(named: "TextColor")
    }
    private let writeTextField = UITextField().then {
        $0.addLeftPadding()
    ...
    override func bind() {
        let input = CommentPlusViewModel.Input(feedId: self.feedId, content: writeTextField.rx.text.orEmpty.asDriver(), postButtonDidTap: doneButton.rx.tap.asSignal())
    ...
    override func setLayout() {
        ...
        doneButton.snp.makeConstraints {
            ...
            $0.top.equalTo(writeTextField.snp.bottom).offset(12)
            $0.height.equalTo(34)
            $0.width.equalTo(68)
        }
